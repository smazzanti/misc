def bq_table_summary(table_name, bq_client, len_quantiles=5, len_topcounts=5):
  ...
  SELECT
    ...
    COUNT(DISTINCT {{COLUMN}}) AS {{COLUMN}}{% if not loop.last %},{% endif %}
    {% endfor -%}
  FROM 
    table_name
  """ \
  ...
  # quantiles
  jinja_query_quantiles = """
  {% set COLUMNS = field_names%}
  SELECT
    {% for COLUMN in COLUMNS -%}
    APPROX_QUANTILES({{COLUMN}}, quantiles) AS {{COLUMN}}{% if not loop.last %},{% endif %}
    {% endfor -%}
  FROM 
    table_name
  """ \
      .replace('field_names', str(field_names)) \
      .replace('table_name', table_name) \
      .replace('quantiles', str(len_quantiles-1))
  output_quantiles = bq_client.query(query=jinja2.Template(jinja_query_quantiles).render()).to_dataframe().iloc[0, :]
  output_quantiles.name = "quantiles"
  ...
  index = ["quantile_{:.0%}".format(x) for x in np.linspace(0,1,len_quantiles)]
  output_quantiles = output_quantiles.apply(lambda x: tmp(x, index))
